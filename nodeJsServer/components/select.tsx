    ...
    ({ options, value, onChange, placeholder = "Select an element", className, }: SelectProps) => {
        ...
        return (
            <div ref={containerRef}
                className={cn(
                    "relative cursor-pointer min-w-[250px] h-[48px] rounded-[4px] flex items-center justify-between px-[24px] bg-background box-content border border-gray-light",
                ...
                <div
                    ...
                    {options.map((option, index) => (
                        <div key={index}
                            className="cursor-pointer flex px-[24px] py-[6px] text-background hover:text-accent hover:bg-background transition"
