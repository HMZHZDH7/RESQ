...
const ChartHistory = () => {
    ...
    return <div className="w-[200px] h-full flex flex-col items-center relative">
        <div className="absolute top-0 left-0 px-[10px] py-[10px] w-full h-full flex flex-col items-center gap-[30px] overflow-y-auto">
            {charts.filter(c => c.image).map((chart, index) => (
                <img
                    key={index}
                    src={chart.image}
                    alt={`Chart ${index}`}
                    className={cn("h-auto w-full object-contain rounded-[4px]", chart === currentChart && "shadow-[0px_0px_6px_0px_#4C98FA]")}
